...
object Int32Spec extends DefaultRunnableSpec {
  val spec = suite("Int32")(
    ...
    suite("Serde")(
      ...
      test("0")(assert(ByteVector.low(4).decodeExactly[Int32])(success(Int32.zero))),
      test("1")(assert((1.toByte +: ByteVector.low(3)).decodeExactly[Int32])(success(Int32.one))),
      test("-1")(assert(ByteVector.fill(4)(0xFF).decodeExactly[Int32])(success(-Int32.one))),
      ...
      test("0xffffff7f == Int32.max")(
        assert((ByteVector.fill(3)(0xFF) :+ 0x7F.toByte).decodeExactly[Int32])(success(Int32.max))
      ),
      test("0x00000080 == Int32.min")(
        assert((ByteVector.low(3) :+ 0x80.toByte).decodeExactly[Int32])(success(Int32.min))
      ),
      test("too large bytevector 0")(assert(ByteVector.low(5).decodeExactly[Int32])(failure)),
      test("too large bytevector 1")(assert(ByteVector.high(5).decodeExactly[Int32])(failure))
    )
  )
}
